...
class Master_settings extends CI_Controller
{
	...
	public function employeeCreation()
	{
		...
		$data['empDetails'] = $this->master_model->fetchCompWiseEmp();
	...
	public function saveEmployeeCreation()
	{
		...
		if (isset($emp_id) && !empty($emp_id) && ($emp_id > 0)) {
			...
			if ($update_emp_result) {
				...
				//  echo '<pre>';print_r($deduction_id);
				// echo '<pre>';print_r($deduction_value);exit;  

				if ((isset($deduction_id) && !empty($deduction_id)) && (isset($deduction_value) && !empty($deduction_value))) {
					// echo "122";
					...
					if (sizeof($deduction_id) > 0) {
						...
						foreach ($deduction_id as $key) {
						   ...
						   if ($key == 4) {
								$deductVal = $this->master_model->selectDetailsWhr('tbl_deduction_allowance', 'deduction_id', $key);
								$dedVal = $deductVal->deduction_default_value;
							} else {
								$dedVal = $deduction_value[$deduct_ins++];
							}
							$deduction_insert_data[] = array(
								'deduction_id' => $key,
								'company_id' => $company_id,
			...
			if ($insert_emp_result) {
				...
				// echo '<pre>';print_r($earning_id);exit;
...
// echo "q";  
// echo '<pre>';print_r($deduction_id);
// echo '<pre>';print_r($deduction_value);
// echo '<pre>';print_r(sizeof($deduction_value));
// echo '<pre>';print_r(sizeof($deduction_id));
				if ((isset($deduction_id) && !empty($deduction_id)) && (isset($deduction_value) && !empty($deduction_value))) {
					$deduction_insert_data = array();
					$deduct_ins = 0;
					if (sizeof($deduction_id) > 0) {
						...
						// echo '<pre>';print_r($deduction_insert_data);
					}
					$deduct_insert_data = $this->master_model->insertBatchSave('tbl_emp_deduct_allowance', $deduction_insert_data);
				}
	...
	public function update_EmpCreation()
	{
		...
		// echo '<pre>';print_r($data['empDetails']);exit;
	...
	public function saveAdvance()
	{
		...
		if (isset($opening) && !empty($opening)) { //print_r($opening);exit();
